      ...
      if (this.job.id) {
        this.jobService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('jhipstertestApp.job.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.jobService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('jhipstertestApp.job.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
